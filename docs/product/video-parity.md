...
| Choices, **I don't know**, and a note area | 8:08–8:30, 10:24–10:33, 11:29–12:04 | Pi supplies one interactive quiz modal; Codex supplies a numbered-card fallback. Answers, admitted gaps, and optional notes commit before feedback. | Pi's published input contract is exercised automatically, but the native modal still requires live human acceptance in the learner's terminal. The Codex surface is chat-native rather than a TUI modal. |
...
| Learner-specific teaching philosophy | 6:53–7:42 | Schema v5 stores a cross-session learner profile with teaching, explanation, feedback, visual, and source preferences. Codex and Pi read it through durable context; Obsidian renders `Profile.md`. | Empty fields deliberately fall back to the shared protocol. |
...
| Learn from learner-supplied material | Product extension | `/teach-from <source> :: <target>` persists a video, PDF, notes file, web page, or repository before the host inspects it. Anchor claims carry exact locators; separate supplemental research can correct, limit, or extend them. Each plan node records source coverage before teaching. | Material access and claim interpretation remain host capabilities. Source coverage is provenance, not evidence that the learner understands the material. |
...
that repository tests can prove. An implemented surface is not a supported
surface until its host-specific live acceptance gate passes.

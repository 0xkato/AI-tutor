...
test("operator quickstart tells a learner what they will see and how to answer", () => {
  ...
  assert.match(quickstart, /first launch[\s\S]*openai-codex[\s\S]*gpt-5\.6-sol/i);
  assert.match(quickstart, /\/model[\s\S]*saved[\s\S]*project[\s\S]*restart/i);
  ...
  assert.match(quickstart, /free-response editor[\s\S]*paste[\s\S]*arrow[\s\S]*Tab[\s\S]*Shift\+Tab/i);
  assert.match(quickstart, /Enter[\s\S]*moves? to the\s+actions[\s\S]*does not submit/i);
  assert.match(quickstart, /Up returns to the answer/i);
  assert.doesNotMatch(quickstart, /Press Down from any editor field/i);
